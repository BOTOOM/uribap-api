...
Tests first: signed-token cases with and without UserInfo, exact subject binding, strict profile shapes, HTTP 200-only success statuses, false/missing verification, signed-name fallback when UserInfo omits names, fresh UserInfo preferred_username precedence over a stale signed name, non-string signed-name rejection, expiration during UserInfo I/O, unauthorized/outage/timeout/redirect behavior, no profile calls for rejected JWTs/scopes, preserved authorization claims, URL validation, seed request configuration and file mode. `/api/v1/health/identity` is a JWKS-availability check, not a claim that optional UserInfo has been probed; live authenticated UserInfo remains a separate acceptance check. Use deterministic synthetic identities and httpx MockTransport; production issuers and SMTP are forbidden in tests.
